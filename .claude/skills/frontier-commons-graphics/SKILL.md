---
name: frontier-commons-graphics
description: Generates on-brand Frontier Commons images (PNG) such as social media posts, Instagram/LinkedIn carousels and stories, quote cards, stat and "did you know" graphics, event and webinar announcements, photo posts, section or cover graphics, LinkedIn banners, email headers and website link previews, using the brand palette, Inter/DM Sans, and the logo. Use whenever someone asks for a Frontier Commons graphic, image, picture, post, flyer, banner, cover, thumbnail, or visual asset, or wants to export the logo as a PNG.
---

# Frontier Commons Graphics
...
3. **Write a spec** (JSON, one object or a list for a batch or carousel) in the user's folder. Start from `examples/demo/all.json`.
4. **Render:**
   ```bash
